...
rasterize_forward_tensor(
    ...
    const torch::Tensor &medium_omega,
    const torch::Tensor &depths,
    ...
    rasterize_forward<<<tile_bounds_dim3, block_dim3>>>(
        ...
        (float3 *)medium_omega.contiguous().data_ptr<float>(),
        depths.contiguous().data_ptr<float>(),
...
nd_rasterize_forward_tensor(
    ...
    nd_rasterize_forward<<<tile_bounds_dim3, block_dim3, shared_mem>>>(
        ...
        medium_omega.contiguous().data_ptr<float>(),
        depths.contiguous().data_ptr<float>(),
        final_Ts.contiguous().data_ptr<float>(),
        final_idx.contiguous().data_ptr<int>(),
        first_idx.contiguous().data_ptr<int>(),
...
std::
    ...
    torch::Tensor v_medium_omega = torch::zeros({img_height, img_width, channels}, xys.options());
    ...
    nd_rasterize_backward_kernel<<<tile_bounds, block, shared_mem>>>(
        ...
        (float *)medium_omega.contiguous().data_ptr<float>(),
        depths.contiguous().data_ptr<float>(),
        background.contiguous().data_ptr<float>(),
        final_Ts.contiguous().data_ptr<float>(),
        final_idx.contiguous().data_ptr<int>(),
        ...
        (float *)v_medium_omega.contiguous().data_ptr<float>()
    );

    return std::make_tuple(v_xy, v_conic, v_colors, v_opacity, v_medium_rgb, v_medium_bs, v_medium_attn, v_medium_omega);
}
...
std::
    ...
    torch::Tensor v_medium_omega = torch::zeros({img_height, img_width, 3}, xys.options());
    ...
    rasterize_backward_kernel<<<tile_bounds, block>>>(
        ...
        *(float3 *)background.contiguous().data_ptr<float>(),
        ...
        (float3 *)v_medium_omega.contiguous().data_ptr<float>()
    );
