...
__global__ void rasterize_forward(
    ...
    for (int b = 0; b < num_batches; ++b) {
        ...
        for (int t = 0; (t < batch_size) && !done; ++t) {
            ...
            // add medium scattering (passive + active)
            float3 exp_bs;
            ...
            float3 passive_intensity;
            passive_intensity.x = exp_bs.x * medium_rgb_pix.x;
            passive_intensity.y = exp_bs.y * medium_rgb_pix.y;
            passive_intensity.z = exp_bs.z * medium_rgb_pix.z;
            ...
            active_intensity.y = medium_omega_pix.y * geom_atten * __expf(-2.0f * medium_attn_pix.y * z_mid) * delta_z;
            active_intensity.z = medium_omega_pix.z * geom_atten * __expf(-2.0f * medium_attn_pix.z * z_mid) * delta_z;

            pix_medium.x = pix_medium.x + T * (passive_intensity.x + active_intensity.x);
            pix_medium.y = pix_medium.y + T * (passive_intensity.y + active_intensity.y);
            pix_medium.z = pix_medium.z + T * (passive_intensity.z + active_intensity.z);
