...
@dataclass
class AbyssSplatModelConfig(ModelConfig):
    """Configuration for the AbyssSplat model."""
...
class AbyssSplatModel(Model):
    ...
    def refinement_after(self, optimizers: Optimizers, step):
        ...
        with torch.no_grad():
            ...
            if do_densification:
                ...
                # 2. 构造射线方向并查询 MLP 计算逐高斯核 beta_D
                dirs_world = self.means - self.last_camera_center
                dirs_world = dirs_world / (dirs_world.norm(dim=-1, keepdim=True) + 1e-6)
                dirs_encoded = self.direction_encoding(dirs_world)
                ...
                # 提取高斯核在三维空间的最大特征尺度
                scales_max = torch.exp(self.scales.detach()).max(dim=-1).values.squeeze()
                # 设定尺度归一化参考系 (例如 0.01，需根据场景实际坐标尺度微调)
                scale_ref = 0.008
                # 构建尺度衰减项，尺度越大，保留率越低
                scale_penalty = torch.clamp(scale_ref / (scales_max + 1e-6), max=1.0)

                # 构建联合置信度
                joint_confidence = sigma_gaussian * scale_penalty
                W_z = T_z + (1.0 - T_z) * joint_confidence
                avg_grad_norm = avg_grad_norm_raw * W_z
    ...
    def get_outputs(self, camera: Cameras, obb_box: Optional[OrientedBox] = None) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        medium_attn = (
            self.sigma_activation(medium_base_out[..., 6:9] + self.medium_density_bias)
            .view(*outputs_shape, -1)
            .to(directions)
        )
        ...
        if self.config.zero_medium:
            ...
            medium_omega = torch.zeros_like(medium_omega)
        ...
        if self.crop_box is not None and not self.training:
            ...
            if crop_ids.sum() == 0:
                ...
                return {"rgb": rgb, "depth": depth, "accumulation": accumulation, "background": medium_rgb,
                        ...
                        "medium_rgb": medium_rgb, "medium_bs": medium_bs, "medium_attn": medium_attn, "medium_omega": medium_omega}
        else:
            crop_ids = None
        ...
        return {"rgb": rgb, "depth": depth_im, "accumulation": alpha, "background": medium_rgb,
                ...
                "medium_rgb": medium_rgb, "medium_bs": medium_bs, "medium_attn": medium_attn, "medium_omega": medium_omega}  # type: ignore
    ...
    def get_metrics_dict(self, outputs, batch) -> Dict[str, torch.Tensor]:
        ...
        for i in range(3):
            ...
            metrics_dict[f"medium_omega_{i}"] = outputs["medium_omega"][:, :, i].mean()
        return metrics_dict
