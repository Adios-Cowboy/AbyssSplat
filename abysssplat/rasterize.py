...
def rasterize_gaussians(
    ...
    medium_omega: Float[Tensor, "*height width channels"],
    img_height: int,
    img_width: int,
    block_width: int,
...
class _RasterizeGaussians(Function):
    ...
    def backward(ctx, v_out_img, v_out_clr, v_out_medium, v_depth_im, v_out_alpha=None):
        ...
        if num_intersects < 1:
            ...
            v_medium_omega = torch.zeros_like(medium_omega)
            ...
            v_xy, v_conic, v_colors, v_opacity, v_medium_rgb, v_medium_bs, v_medium_attn, v_medium_omega = rasterize_fn(
                img_height,
                img_width,
                ctx.block_width,
                gaussian_ids_sorted,
